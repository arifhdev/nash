                ...
                <div class="bg-white rounded-2xl p-8 shadow-sm border border-gray-200 min-h-[700px]">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        @forelse($courses as $course)
                            ...
                            {{-- Course Item Card --}}
                            <div class="group flex flex-col bg-white rounded-2xl border border-gray-200 overflow-hidden {{ $isLocked ? 'opacity-90' : 'hover:border-red-200 hover:shadow-lg hover:shadow-red-500/5' }} transition-all duration-300">
                                
                                {{-- Thumbnail Link --}}
                                @if($isLocked)
                                    <div class="block relative h-48 bg-gray-100 overflow-hidden cursor-not-allowed">
                                @else
                                    <a href="{{ route('course.detail', $course->slug) }}" wire:navigate class="block relative h-48 bg-gray-100 overflow-hidden">
                                @endif
                                
                                    @if($course->image)
                                        <img src="{{ Storage::url($course->image) }}" class="w-full h-full object-cover {{ $isLocked ? 'grayscale' : 'group-hover:scale-105' }} transition-transform duration-500" alt="{{ $course->title }}">
                                    @else
                                    ...
                                    <div class="absolute top-4 right-4 z-10">
                                        @if($isLocked)
                                            <span class="inline-flex items-center px-2.5 py-1 rounded-md text-[10px] font-bold uppercase tracking-wider bg-gray-800 text-white shadow-sm border border-gray-700 backdrop-blur-sm">
                                                Terkunci
                                            </span>
                                ...
                                <div class="flex-1 p-6 flex flex-col justify-between">
                                    <div>
                                        @if($isLocked)
                                            <h3 class="text-lg font-bold text-gray-500 leading-tight mb-2 line-clamp-2 cursor-not-allowed">
                                                {{ $course->title }}
                                            </h3>
                                        @else
                                        ...
                                        @if($isLocked)
                                            <div class="mt-3 mb-4 p-3 bg-red-50 rounded-xl border border-red-100 flex-grow">
                                                ...
                                                <ul class="list-disc list-inside text-red-600 space-y-1 text-xs">
                                                    @foreach($course->prerequisites as $req)
                                                        @if(!auth()->user()->hasCompletedCourse($req->id))
                                                            <li class="line-clamp-1" title="{{ $req->title }}">{{ $req->title }}</li>
                                                        @endif
                                                    @endforeach
                                                </ul>
                                            </div>
                                        @else
                                            ...
                                            <div class="text-gray-500 text-sm leading-relaxed mb-4 line-clamp-2 prose prose-sm max-w-none 
                                    ...
                                    <div class="mt-6 flex items-center justify-between border-t border-gray-50 pt-4">
                                        ...
                                        @if($isLocked)
                                            <span class="inline-flex items-center text-xs font-black text-gray-400 bg-gray-100 px-4 py-2 rounded-lg cursor-not-allowed uppercase tracking-wider shadow-sm border border-gray-200">
                                                Terkunci
                                            </span>
                                        @else
