        ...
        <form wire:submit="register" class="space-y-5">
            ...
            @if($user_type)
                <div class="p-5 bg-gray-50 rounded-lg border border-gray-200 space-y-5 animate-fade-in-down">
                    ...
                    {{-- INFO OTOMATIS: Penempatan & Jabatan --}}
                    @if($main_dealer_id || $dealer_id || $position_id)
                        <div class="animate-fade-in-down space-y-4 pt-4 border-t border-gray-200 mt-4">
                            {{-- Main Dealer Muncul untuk Dealer & Main Dealer (Trainer/Non-Trainer) --}}
                            @if($main_dealer_id)
                                <div>
                            ...
                            {{-- Dealer & Jabatan HANYA muncul jika statusnya Karyawan Dealer --}}
                            @if($user_type === 'dealer')
                                @if($dealer_id)
                                    <div>
            ...
            @if($hasValidId)
                {{-- Form Lanjutan (Nama, Email, HP, Password) --}}
                <div class="grid grid-cols-1 md:grid-cols-2 gap-5 animate-fade-in-up">
                    ...
                    <div>
                        ...
                        <input wire:model="email" type="email" class="w-full pl-4 py-3 bg-white border border-gray-200 rounded-lg text-sm focus:outline-none focus:border-[#ED1C24] focus:ring-1 focus:ring-[#ED1C24]" placeholder="email@anda.com" required>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-5 animate-fade-in-up">
                     <div>
                        ...
                        <input wire:model="password" type="password" class="w-full pl-4 py-3 bg-white border border-gray-200 rounded-lg text-sm focus:outline-none focus:border-[#ED1C24] focus:ring-1 focus:ring-[#ED1C24]" placeholder="••••••••" required>
                        @error('password') <span class="text-red-500 text-xs font-bold mt-1 block">{{ $message }}</span> @enderror
                     </div>
                     <div>
                        <label class="block font-bold text-sm text-gray-700 mb-1">Konfirmasi Password</label>
                        <input wire:model="password_confirmation" type="password" class="w-full pl-4 py-3 bg-white border border-gray-200 rounded-lg text-sm focus:outline-none focus:border-[#ED1C24] focus:ring-1 focus:ring-[#ED1C24]" placeholder="••••••••" required>
                     </div>
                </div>
